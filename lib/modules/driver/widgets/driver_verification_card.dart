...
/// Max width so the license keeps phone-like proportions on web/tablet.
const double kDriverLicenseCardMaxWidth = 420;

/// Centers [child] and caps width for ID-card proportions on wide layouts.
///
/// Uses [Align.heightFactor] so this is safe inside a vertical [ListView]
/// (plain [Align]/[Center] would expand to infinite height).
Widget _licenseCardFrame({required Widget child}) {
  return Align(
...
class _PhotoColumn extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final photoWidth = compact ? 64.0 : 88.0;
    final path = driver.photoPath;
    final bytes = PhotoStorage.decode(path);
